...
Class ChineseName
{
     ...
     * @return mixed
     */
    private static function generateSurname()
    {
        return self::$surnames[mt_rand(0,count(self::$surnames)-1)];
    ...
    {
        return self::$firstNames[mt_rand(0,count(self::$firstNames)-1)];
    }
     ...
     * @return array
     */
    public static function generate($model=0)
    {
        switch($model)
        {
            case 1:
                // 单字名
                $name = self::generateSurname().self::generateFirstName();
                break;
            case 2:
                // 1姓+2位名
                $name = self::generateSurname().self::generateFirstName().self::generateFirstName();
                break;
            case 3:
                // 只取姓
                $name = self::generateSurname();
                break;
            case 4:
                // 只取名
                $name = self::generateFirstName();
                break;
            case 0:
            default:
                // 默认：随机字数名（1-2位）
                $name = self::generateSurname().self::generateFirstName();
                mt_rand(0,100)>50 && $name .= self::generateFirstName();
        }
        $pinyin = new Pinyin();
        $pinyin = implode('', $pinyin->name($name));
        $return = [
            'name' => $name,
            'pinyin' => $pinyin
        ];
